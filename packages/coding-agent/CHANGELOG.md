...
- Excluded the experimental `computer` desktop-control tool and the opt-in `gjc rlm` research mode from the 0.5.4 patch release. Their CLI/tool registrations, bundled prompts, native `ComputerController`/`computerScreenshot` exports, settings/docs, and tests were removed from this release branch so 0.5.4 ships only the stabilized dev-branch fixes without the computer-use and rlm feature surfaces.
