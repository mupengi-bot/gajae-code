...
#[inline]
fn is_js_trim_whitespace(unit: u16) -> bool {
    matches!(
        unit,
...
#[inline]
fn is_ascii_ws(unit: u16) -> bool {
    matches!(unit, 0x0009 | 0x000A | 0x000B | 0x000C | 0x000D | 0x0020)
}
...
fn normalize_block_lines(lines: &[Line<'_>], include_depth: bool) -> Vec<Vec<u16>> {
    let depths = if include_depth { Some(relative_indent_depths(lines)) } else { None };
    lines
        .iter()
        .enumerate()
        .map(|(idx, &line)| {
...
fn levenshtein(a: &[u16], b: &[u16]) -> usize {
    ...
    for i in 1..=a_len {
        ...
        let a_code = a[i - 1];
        for j in 1..=b_len {
            let cost = if a_code == b[j - 1] { 0 } else { 1 };
...
fn best_core_one_line(
    ...
    for (start, &line) in content_lines.iter().enumerate() {
        ...
        if score >= threshold { above_threshold_count += 1; }
        if score > best_score {
...
#[napi]
pub fn h01_find_best_fuzzy_match(content: JsString, target: JsString, threshold: f64) -> Result<H01BestFuzzyMatchResult> {
    ...
    if target_lines.is_empty() || target_units.is_empty() || target_lines.len() > content_lines.len() {
        return Ok(H01BestFuzzyMatchResult { best: None, above_threshold_count: 0, second_best_score: 0.0 });
    }
...
#[napi]
pub fn h02_score_sequence_fuzzy(lines: Vec<String>, pattern: Vec<String>, start: u32, eof: bool) -> H02SequenceFuzzyResult {
    ...
    if pattern_units.is_empty() || pattern_units.len() > line_units.len() {
        return H02SequenceFuzzyResult { index: None, confidence: 0.0, match_count: 0, match_indices: Vec::new(), second_best_score: 0.0 };
    }
    let max_start = line_units.len() - pattern_units.len();
    let start = start as usize;
    let search_start = if eof && line_units.len() >= pattern_units.len() { max_start } else { start };
    ...
    let mut score_range = |from: usize, to: usize| {
        for i in from..=to {
            ...
            if score >= SEQUENCE_FUZZY_THRESHOLD {
                if first_match.is_none() { first_match = Some(i as u32); }
                match_count += 1;
                if match_indices.len() < MAX_RECORDED_MATCHES { match_indices.push(i as u32); }
            }
            if score > best_score {
                second_best_score = best_score;
                best_score = score;
    ...
    if search_start <= max_start { score_range(search_start, max_start); }
    if eof && search_start > start { score_range(start, search_start - 1); }
    H02SequenceFuzzyResult { index: best_index, confidence: best_score, match_count, match_indices, second_best_score }
}
